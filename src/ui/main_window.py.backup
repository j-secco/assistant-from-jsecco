...
class MainWindow(QMainWindow):
    ...
    def _create_center_panel(self, layout: QHBoxLayout):
        """Create the center panel with position display and controls."""
        center_panel = QFrame()
        ...
        center_layout.addWidget(tcp_group)
        ...
        for i, coord in enumerate(tcp_coords):
            ...
            value = QLabel("0.00")
            value.setStyleSheet("font-size: 16px; color: #2196F3;")
            
            tcp_layout.addWidget(label, i//2, (i%2)*2)
            tcp_layout.addWidget(value, i//2, (i%2)*2 + 1)
            self.position_labels[coord] = value
        
        # Joint Angles Display
        joint_group = QGroupBox("Joint Angles")
        center_layout.addWidget(joint_group)
