                ...
                if self.websocket_controller and self.websocket_controller.is_connected():
                    ...
                    try:
                        ...
                        if self.websocket_receiver and self.websocket_receiver.is_connected():
                            ...
                            # Use last known position when not connected
                            tcp_pose = self.robot_status.get('tcp_pose', [0.0] * 6)
                            joint_angles = self.robot_status.get('joint_angles', [0.0] * 6)
                        
                        # Update robot status with current position data
                        self.robot_status['tcp_pose'] = tcp_pose
                        self.robot_status['joint_angles'] = joint_angles
                        
                        # Notify position callbacks
                        self._on_position_update(tcp_pose, joint_angles)
