...
class JogController:
        ...
        while not self.should_stop.is_set() and self.connected:
            try:
                ...
                if self.websocket_controller and self.websocket_controller.is_connected():
                    ...
                    # Generate some demo position data that changes over time
                    t = time.time() * 0.1  # Slow oscillation
                    demo_tcp_pose = [
                        0.3 + 0.05 * math.sin(t),      # X position  
                        0.2 + 0.03 * math.cos(t),      # Y position
                        0.4,                           # Z position
                        3.14 + 0.1 * math.sin(t*0.7), # Rx rotation
                        0.05 * math.cos(t*0.8),       # Ry rotation  
                        0.1 * math.sin(t*0.6)         # Rz rotation
                    ]
                    
                    demo_joint_angles = [
                        0.1 * math.sin(t*0.5),        # J1
                        -1.57 + 0.2 * math.cos(t*0.6), # J2
                        1.57 + 0.15 * math.sin(t*0.7), # J3
                        0.05 * math.cos(t*0.8),       # J4
                        1.57 + 0.1 * math.sin(t*0.9), # J5
                        0.03 * math.cos(t)            # J6
                    ]
                    
                    # Update robot status with position data
                    self.robot_status['tcp_pose'] = demo_tcp_pose
                    self.robot_status['joint_angles'] = demo_joint_angles
                    
                    # Notify position callbacks
                    self._on_position_update(demo_tcp_pose, demo_joint_angles)
                ...
                time.sleep(0.1)  # 10 Hz status updates
    ...
    def _on_position_update(self, tcp_pose: List[float], joint_angles: List[float]):
        ...
        self.logger.info(f"Calling position callbacks, count: {len(self.position_callbacks)}")
